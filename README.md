...
- **REST API (OpenAPI 3.0)** - для синхронных запросов, где вызывающей стороне нужен немедленный ответ: операции пользователя через API Gateway (регистрация устройства, чтение состояния и телеметрии) и межсервисные вызовы (сервис сценариев отправляет команду в сервис устройств).
- **AsyncAPI (Kafka)** - для асинхронных взаимодействий, где ответ сразу не нужен: поток телеметрии от устройств, доставка команд до IoT-шлюза, подтверждения статусов. Шина буферизует пиковые нагрузки и позволяет сервисам работать независимо - если сервис телеметрии недоступен, показания не теряются, а копятся в топике.

Граница проходит так: всё, что инициирует пользователь и ждет ответа - REST. Всё, что связано с потоком событий от устройств и к устройствам - шина.
